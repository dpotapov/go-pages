...
type FragmentState struct {
	Fragment string
	State    FragmentRenderStatus
}

// FragmentRenderStatus represents the current state of fragment processing
type FragmentRenderStatus int

const (
	// FragmentRendering indicates we're rendering inside the fragment
	FragmentRendering FragmentRenderStatus = iota
	// FragmentSearching indicates we're looking for the fragment
	FragmentSearching
	// FragmentCompleted indicates we've finished rendering the fragment
	FragmentCompleted
)

// fragmentScope wraps any Scope, carries a requested fragment ID and
// a shared, mutable FragmentState (Searching→Rendering→Completed).
type fragmentScope struct {
	inner Scope
	state *FragmentState
}

func NewFragmentScope(inner Scope, fragment string) Scope {
	if fragment == "" {
		return inner
	}
	return &fragmentScope{
		...
		state: &FragmentState{
			Fragment: fragment,
			State:    FragmentSearching,
		},
	}
}

// Scope methods
func (f *fragmentScope) Spawn(vars map[string]any) Scope {
	child := f.inner.Spawn(vars)
	// only wrap children while still searching
	if f.state.State != FragmentSearching {
		return child
	}
	return &fragmentScope{inner: child, state: f.state}
}
func (f *fragmentScope) Vars() map[string]any { return f.inner.Vars() }
func (f *fragmentScope) Touch()               { f.inner.Touch() }
func (f *fragmentScope) DryRun() bool         { return f.inner.DryRun() }

// ScopeFragment
func (f *fragmentScope) Fragment() *FragmentState {
	return f.state
}
