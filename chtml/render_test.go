...
func TestRenderCHTMLFragments(t *testing.T) {
	tests := []struct {
		name           string
		text           string
		fragment       string
		...
		{
			name: "no fragment requested",
			text: `<div id="frag1">Frag 1</div><div id="frag2">Frag 2</div>`,
			want: `<div id="frag1">Frag 1</div><div id="frag2">Frag 2</div>`,
		},
		{
			name:     "render existing fragment 1",
			text:     `<div id="frag1">Frag 1</div><div id="frag2">Frag 2</div>`,
			fragment: "frag1",
			want:     `Frag 1`,
		},
		{
			name:     "render existing fragment 2",
			text:     `<div id="frag1">Frag 1</div><div id="frag2">Frag 2</div>`,
			fragment: "frag2",
			want:     `Frag 2`,
		},
		{
			name:     "fragment ID not found",
			text:     `<div id="frag1">Frag 1</div><div id="frag2">Frag 2</div>`,
			fragment: "frag3",
			want:     nil, // Expect empty result as fragment wasn't found
		},
		{
			name:     "nested fragments - select outer",
			text:     `<div id="outer"><p>Outer</p><div id="inner">Inner</div></div>`,
			fragment: "outer",
			want:     `<p>Outer</p><div id="inner">Inner</div>`,
		},
		{
			name:     "nested fragments - select inner",
			text:     `<div id="outer"><p>Outer</p><div id="inner">Inner</div></div>`,
			fragment: "inner",
			want:     `Inner`,
		},
		{
			name:     "fragment inside c:if - true",
			text:     `<div c:if="true"><span id="frag">Inside If</span></div>`,
			fragment: "frag",
			want:     `Inside If`,
		},
		{
			name:     "fragment inside c:if - false",
			text:     `<div c:if="false"><span id="frag">Inside If</span></div>`,
			fragment: "frag",
			want:     nil,
		},
		{
			name:     "fragment inside c:for",
			text:     `<ul><li c:for="i in [1, 2, 3]" id="item-${i}">Item ${i}</li></ul>`,
			fragment: "item-2",
			want:     `Item 2`,
		},
		{
			name:     "fragment inside c:for - ID not generated",
			text:     `<ul><li c:for="i in [1, 2, 3]" id="item-${i}">Item ${i}</li></ul>`,
			fragment: "item-4",
			want:     nil,
		},
		{
			name:     "fragment wrapping c:for",
			text:     `<div id="list"><ul c:for="i in [1, 2]"><li>Item ${i}</li></ul></div>`,
			fragment: "list",
			want:     `<ul><li>Item 1</li></ul><ul><li>Item 2</li></ul>`,
		},
		{
			name:     "fragment inside import",
			text:     `<c:comp-with-fragment />`,
			fragment: "frag-in-comp",
			want:     `Fragment inside component`,
		},
		{
			name:     "import inside fragment",
			text:     `<div id="frag-outer"><c:comp1 /></div>`,
			fragment: "frag-outer",
			want:     `<p>comp1</p>`,
		},
		{
			name:           "fragment selection stops rendering - causing error",
			text:           `<div id="first">First</div><div id="second">${ 1/0 }</div>`,
			fragment:       "first",
			want:           `First`,
			wantRenderErrs: 0,
		},
		{
			name:     "deeply nested fragment",
			text:     `<div><div><span id="deep">Deep</span></div></div>`,
			fragment: "deep",
			want:     `Deep`,
		},
		{
			name:     "Fragment ID within text node (should not match)",
			text:     `<p>Some text with <span id="real">id</span> in it</p>`,
			fragment: "id",
			want:     nil,
		},
		{
			name:     "Fragment ID as part of attribute value (should not match)",
			text:     `<div data-target="#frag"><p id="real">Real</p></div>`,
			fragment: "frag",
			want:     nil,
		},
		{
			name:     "Fragment with evaluated ID",
			text:     `<c:attr name="myid">frag</c:attr><div id="${myid}-1">Content</div>`,
			fragment: "frag-1",
			want:     `Content`,
		},
	}
	...
	imp := &testImporter{}
	imp.init()
	imp.add("comp-with-fragment", `<div id="frag-in-comp">Fragment inside component</div>`)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := testRenderCaseWithFragment(tt.text, tt.want, nil, opts, tt.fragment); err != nil {
				var compErr *ComponentError
				if errors.As(err, &compErr) {
					if tt.wantRenderErrs > 0 {
						return
					} else {
						t.Errorf("unexpected component error: %v\nContext:\n%s\nStack:\n%s",
							compErr, compErr.HTMLContext(), compErr.StackTrace())
					}
				} else {
					if tt.wantRenderErrs == 0 {
						t.Errorf("unexpected non-component error: %v", err)
					} else {
						t.Logf("expected a component error, but got a different error type: %v", err)
					}
				}
			} else if tt.wantRenderErrs > 0 {
				t.Errorf("expected %d component errors, but got nil error", tt.wantRenderErrs)
			}
		})
	}
}
...
func testRenderCaseWithFragment(text string, want any, vars map[string]any, opts *ComponentOptions, fragmentID string) (err error) {
	var imp Importer
	if opts != nil {
		imp = opts.Importer
	}
	...
	// Create the appropriate scope
	s := NewFragmentScope(NewBaseScope(vars), fragmentID)

	rr, err := comp.Render(s) // Pass the scope (either BaseScope or scopeFragment)
	if err != nil {
		return fmt.Errorf("render error: %w", err)
	}
