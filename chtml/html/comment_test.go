...
func testOneComment(t *testing.T, b []byte) {
	...
	if next := z.Next(); next != html.CommentToken {
		t.Fatalf("Next(%q): got %v, want %v", b, next, html.CommentToken)
	}
...
loop:
	...
	z2 := html.NewTokenizer(strings.NewReader(tok.String()))
	if next := z2.Next(); next != html.CommentToken {
		t.Fatalf("round-trip Next(%q): got %v, want %v", suffix, next, html.CommentToken)
	}
